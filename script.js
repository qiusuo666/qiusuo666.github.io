...
function saveState() {
    ...
    if (isLoggedIn && !isSyncing) { clearTimeout(syncDebounceTimer); syncDebounceTimer = setTimeout(() => uploadToCloud(), SYNC_DEBOUNCE); }
}

function loadState() {
    try { const raw = localStorage.getItem('cst_data'); if (raw) { const d = JSON.parse(raw); if (d.categories && d.categories.length > 0) STATE.categories = d.categories; if (d.history) STATE.history = d.history; if (d.currentEngine !== undefined) STATE.currentEngine = d.currentEngine; if (d.favorites) STATE.favorites = d.favorites; return; } } catch (e) {}
    STATE.currentEngine = 0; STATE.history = [];
    STATE.categories = [{ id: STATE.defaultCategoryId, name: '常用', links: [{ id: 'd1', name: 'GitHub', url: 'https://github.com' },{ id: 'd2', name: '哔哩哔哩', url: 'https://www.bilibili.com' },{ id: 'd3', name: 'YouTube', url: 'https://www.youtube.com' },{ id: 'd4', name: 'Twitter', url: 'https://twitter.com' },{ id: 'd5', name: 'Reddit', url: 'https://www.reddit.com' },{ id: 'd6', name: 'Stack Overflow', url: 'https://stackoverflow.com' },{ id: 'd7', name: 'Wikipedia', url: 'https://www.wikipedia.org' },{ id: 'd8', name: 'Gmail', url: 'https://mail.google.com' }] }];
}
...
function renderEngines() {
    ...
    currentEngineIcon.src = selected.icon; currentEngineIcon.onerror = function() { this.style.display = 'none'; };
    currentEngineName.textContent = selected.name;
    engineDropdown.innerHTML = ENGINES.map((engine, i) => `<div class="engine-option ${i === STATE.currentEngine ? 'active' : ''}" data-index="${i}"><img src="${engine.icon}" alt="" onerror="this.style.display='none'"><span>${engine.name}</span></div>`).join('');
    engineDropdown.querySelectorAll('.engine-option').forEach(el => { el.addEventListener('click', (e) => { e.stopPropagation(); STATE.currentEngine = parseInt(el.dataset.index, 10); saveState(); renderEngines(); closeEngineDropdown(); searchInput.focus(); }); });
}
...
function doSearch(query) {
    if (!query || !query.trim()) return; const q = query.trim();
    addHistory(q); window.open(ENGINES[STATE.currentEngine].url + encodeURIComponent(q), '_blank');
    searchInput.value = ''; closeSuggestions();
}
...
function renderHistory() {
    ...
    historyTags.innerHTML = STATE.history.map(q => `<div class="history-tag" data-query="${q}"><span>${q}</span><span class="remove-history" data-query="${q}"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg></span></div>`).join('');
    historyTags.querySelectorAll('.history-tag').forEach(el => { el.addEventListener('click', (e) => { if (e.target.closest('.remove-history')) return; doSearch(el.dataset.query); }); });
    historyTags.querySelectorAll('.remove-history').forEach(el => { el.addEventListener('click', (e) => { e.stopPropagation(); removeHistory(el.dataset.query); }); });
}
...
searchInput.addEventListener('input', () => { clearTimeout(suggestTimer); const val = searchInput.value.trim(); if (!val) { closeSuggestions(); return; } suggestTimer = setTimeout(() => { const matches = STATE.history.filter(h => h.includes(val) && h !== val); matches.length > 0 ? showSuggestions(matches) : closeSuggestions(); }, 200); });

function showSuggestions(items) { suggestions.innerHTML = items.map(q => `<div class="suggestion-item" data-query="${q}"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg><span>${q}</span></div>`).join(''); suggestions.classList.add('active'); suggestions.querySelectorAll('.suggestion-item').forEach(el => { el.addEventListener('mousedown', (e) => { e.preventDefault(); doSearch(el.dataset.query); }); }); }
function closeSuggestions() { suggestions.classList.remove('active'); suggestions.innerHTML = ''; }

function showMainView() { mainView.classList.remove('hidden'); linksView.classList.remove('active'); closeContextMenu(); wallpaperImg.classList.remove('blurred'); document.querySelector('.wallpaper').classList.remove('overlay-strong'); }
function showLinksView() { mainView.classList.add('hidden'); linksView.classList.add('active'); closeContextMenu(); wallpaperImg.classList.add('blurred'); document.querySelector('.wallpaper').classList.add('overlay-strong'); }

document.addEventListener('contextmenu', (e) => { if (modalOverlay.classList.contains('active')) return; if (e.target.closest('.main-view') && !e.target.closest('input') && !e.target.closest('.search-engine-selector')) { e.preventDefault(); showLinksView(); } });
document.addEventListener('mousedown', (e) => { if (modalOverlay.classList.contains('active')) return; if (linksView.classList.contains('active') && (e.target === linksView || e.target === linksContainer || e.target.closest('.links-view-bg'))) { showMainView(); } });
...
function handleDropOnCategory(targetCatId) { if (!STATE.dragData) return; const fromCat = STATE.categories.find(c => c.id === STATE.dragData.catId); const toCat = STATE.categories.find(c => c.id === targetCatId); if (!fromCat || !toCat || STATE.dragData.catId === targetCatId) return; const li = fromCat.links.find(l => l.id === STATE.dragData.linkId); if (!li) return; fromCat.links = fromCat.links.filter(l => l.id !== STATE.dragData.linkId); toCat.links.push({ ...li }); saveState(); renderLinks(); }
...
modalConfirm.addEventListener('click', () => { if (linkForm.style.display !== 'none') { const name = linkNameInput.value.trim(); let url = linkUrlInput.value.trim(); if (!name || !url) return; if (!url.startsWith('http://') && !url.startsWith('https://')) url = 'https://' + url; try { new URL(url); } catch { return; } addOrEditLink(name, url); } else { const name = catNameInput.value.trim(); if (!name) return; if (STATE.editCatId) { saveCatName(name); } else { STATE.categories.push({ id: generateId(), name, links: [] }); saveState(); renderLinks(); closeModal(); } } });
...
(function() {
    ...
    let w, h, mx = -9999, my = -9999, prevMx = -9999, prevMy = -9999, stillTime = 0;
    const PARTICLES = 100, REPEL_RADIUS = 120, MAX_REPEL = 2.8, DAMPING = 0.92, EDGE_BOUNCE = 0.3, STILL_THRESHOLD = 400, TIDE_INNER = 200, TIDE_OUTER = 320, TIDE_PERIOD = 2.5, TIDE_STRENGTH = 0.18, TIDE_FADE = 0.93;
    let tideFactor = 0;
    const particles = [];
    ...
    class Particle {
        constructor() {
            this.size = rand(6, 22); this.opacity = rand(0.06, 0.2);
            this.x = rand(0, w || window.innerWidth); this.y = rand(0, h || window.innerHeight);
        ...
        update(isTidal, time) {
            if (mx > -9000 && my > -9000) {
                ...
                if (dist < REPEL_RADIUS && dist > 0.5) {
                    const force = ((REPEL_RADIUS - dist) / REPEL_RADIUS) * MAX_REPEL, nx = dx / dist, ny = dy / dist;
                    this.repelVx += nx * force * (1 - tideFactor * 0.85); this.repelVy += ny * force * (1 - tideFactor * 0.85);
                }
                if (isTidal && dist < TIDE_OUTER && dist > 0.5) {
                    const phase = Math.sin(time * (2 * Math.PI / TIDE_PERIOD)), nx = dx / dist, ny = dy / dist;
                    let strength = TIDE_STRENGTH * phase * tideFactor;
                    if (dist > TIDE_INNER) strength *= Math.max(0, 1 - (dist - TIDE_INNER) / (TIDE_OUTER - TIDE_INNER)) * 0.5 + 0.3;
                    this.repelVx += nx * strength; this.repelVy += ny * strength;
                }
            }
            this.x += this.vx + this.repelVx; this.y += this.vy + this.repelVy;
            this.repelVx *= DAMPING; this.repelVy *= DAMPING;
            if (Math.abs(this.repelVx) < 0.003) this.repelVx = 0; if (Math.abs(this.repelVy) < 0.003) this.repelVy = 0;
    ...
    document.addEventListener('mousemove', (e) => { if (Math.abs(e.clientX - prevMx) > 0.5 || Math.abs(e.clientY - prevMy) > 0.5) stillTime = 0; mx = e.clientX; my = e.clientY; prevMx = mx; prevMy = my; });
    ...
    resize(); for (let i = 0; i < PARTICLES; i++) particles.push(new Particle());

    let lastTime = performance.now(), animTime = 0;
    function draw(now) {
        const dt = now - lastTime; lastTime = now; animTime += dt / 1000;
        if (mx > -9000) { stillTime += dt; if (stillTime >= STILL_THRESHOLD) { tideFactor += (1 - tideFactor) * 0.03; if (tideFactor > 0.99) tideFactor = 1; } }
        if (stillTime < STILL_THRESHOLD && tideFactor > 0.001) { tideFactor *= TIDE_FADE; if (tideFactor < 0.001) tideFactor = 0; }
        ctx.clearRect(0, 0, w, h);
        for (const p of particles) { p.update(tideFactor > 0.01, animTime); p.draw(ctx); }
...
async function uploadToCloud() {
    ...
    isSyncing = true;
    try { await window.CST.uploadData(getSyncData()); lastSyncTime = new Date(); updateSyncStatus(); } catch (e) { userDropdownSync.textContent = '⚠ 同步失败'; userDropdownSync.className = 'user-dropdown-sync error'; }
...
async function downloadFromCloud() {
    ...
    try {
        ...
        if (data) { applyCloudData(data); renderLinks(); renderFavorites(); renderHistory(); renderEngines(); loadWallpaper(); showSyncToast('已从云端下载数据 ✓', 'success'); }
        else { showSyncToast('云端暂无数据，已上传本地数据', 'success'); await uploadToCloud(); }
...
function updateSyncStatus() {
    ...
    if (lastSyncTime) { const diff = Math.round((new Date() - lastSyncTime) / 1000); const min = Math.floor(diff / 60); userDropdownSync.textContent = min > 0 ? `✓ ${min}分钟前已同步` : '✓ 已同步'; userDropdownSync.className = 'user-dropdown-sync synced'; }
    else { userDropdownSync.textContent = '○ 未同步'; userDropdownSync.className = 'user-dropdown-sync'; }
}
...
function openAuthModal(mode) {
    ...
    authConfirmPassword.value = ''; authOldPassword.value = ''; authNewPassword.value = '';
    if (mode === 'login') { authModalTitle.textContent = '登录'; authModalSubtitle.textContent = '登录后可在多端同步数据'; authSubmitBtn.textContent = '登录'; authSwitchText.textContent = '还没有账号？'; authSwitchBtn.textContent = '注册'; authConfirmPasswordGroup.style.display = 'none'; authOldPasswordGroup.style.display = 'none'; authNewPasswordGroup.style.display = 'none'; authRememberLabel.style.display = 'flex'; }
    else if (mode === 'register') { authModalTitle.textContent = '注册'; authModalSubtitle.textContent = '创建账号即可多端同步'; authSubmitBtn.textContent = '注册'; authSwitchText.textContent = '已有账号？'; authSwitchBtn.textContent = '登录'; authConfirmPasswordGroup.style.display = 'flex'; authOldPasswordGroup.style.display = 'none'; authNewPasswordGroup.style.display = 'none'; authRememberLabel.style.display = 'none'; }
    else if (mode === 'changePassword') { authModalTitle.textContent = '修改密码'; authModalSubtitle.textContent = '请输入旧密码和新密码'; authSubmitBtn.textContent = '保存'; authSwitchText.textContent = ''; authSwitchBtn.textContent = ''; authConfirmPasswordGroup.style.display = 'none'; authOldPasswordGroup.style.display = 'flex'; authNewPasswordGroup.style.display = 'flex'; authRememberLabel.style.display = 'none'; }
    authModalOverlay.classList.add('active'); setTimeout(() => authUsername.focus(), 200);
}

function closeAuthModal() { authModalOverlay.classList.remove('active'); authMode = 'login'; }
function showAuthError(msg) { authError.textContent = msg; authError.classList.add('visible'); setTimeout(() => authError.classList.remove('visible'), 4000); }

authBtn.addEventListener('click', (e) => { e.stopPropagation(); isLoggedIn ? userDropdown.classList.toggle('open') : openAuthModal('login'); });
document.addEventListener('click', (e) => { if (!e.target.closest('.user-dropdown') && !e.target.closest('.auth-btn')) userDropdown.classList.remove('open'); });
$('authModalClose').addEventListener('click', closeAuthModal);
authModalOverlay.addEventListener('mousedown', (e) => { if (e.target === authModalOverlay) closeAuthModal(); });
authSwitchBtn.addEventListener('click', () => { authMode === 'login' ? openAuthModal('register') : openAuthModal('login'); });
...
authSubmitBtn.addEventListener('click', async () => {
    ...
    if (authMode === 'changePassword') {
        const oldPwd = authOldPassword.value.trim(), newPwd = authNewPassword.value.trim();
        ...
        try {
            authSubmitBtn.disabled = true; authSubmitBtn.textContent = '保存中...';
            const client = window.CST.getSupabase();
            const { error: signInErr } = await client.auth.signInWithPassword({ email: username.toLowerCase() + '@custom-tabs.local', password: oldPwd });
            if (signInErr) { showAuthError('旧密码不正确'); authSubmitBtn.disabled = false; authSubmitBtn.textContent = '保存'; return; }
            await window.CST.changePassword(newPwd); closeAuthModal(); showSyncToast('密码修改成功 ✓', 'success');
        } catch (e) { showAuthError(e.message || '密码修改失败'); }
        authSubmitBtn.disabled = false; authSubmitBtn.textContent = '保存'; return;
    }
    if (!password) { showAuthError('请输入密码'); return; }
    if (password.length < 6) { showAuthError('密码至少6位'); return; }
    if (authMode === 'register' && password !== authConfirmPassword.value.trim()) { showAuthError('两次密码不一致'); return; }
    try {
        authSubmitBtn.disabled = true; authSubmitBtn.textContent = authMode === 'login' ? '登录中...' : '注册中...';
        let result; const remember = authRememberMe.checked;
        ...
        onLoginSuccess(result.user, { download: true }); closeAuthModal(); showSyncToast('登录成功 ✓，数据将在操作后自动同步', 'success');
    } catch (e) {
        ...
        showAuthError(msg);
    } finally { authSubmitBtn.disabled = false; authSubmitBtn.textContent = authMode === 'login' ? '登录' : '注册'; }
});

function onLoginSuccess(user, { download = false } = {}) {
    isLoggedIn = true; currentUser = user;
    authBtn.classList.add('logged-in'); authBtn.title = window.CST.getDisplayName(user);
    userDropdownName.textContent = window.CST.getDisplayName(user);
    updateSyncStatus(); if (download) setTimeout(() => downloadFromCloud(), 500);
}

$('btnSignOut').addEventListener('click', async () => {
    if (!confirm('确定退出登录？本地数据会保留。')) return;
    try { await window.CST.signOut(); isLoggedIn = false; currentUser = null; lastSyncTime = null; authBtn.classList.remove('logged-in'); authBtn.title = '登录'; userDropdown.classList.remove('open'); userDropdownSync.textContent = '○ 未同步'; userDropdownSync.className = 'user-dropdown-sync'; showSyncToast('已退出登录', ''); } catch (e) {}
});

$('btnSyncUpload').addEventListener('click', async () => {
    userDropdown.classList.remove('open'); if (!isLoggedIn) { showSyncToast('请先登录', 'error'); return; }
    isSyncing = true;
    try { await window.CST.uploadData(getSyncData()); lastSyncTime = new Date(); updateSyncStatus(); showSyncToast('已上传到云端 ✓', 'success'); }
    catch (e) { showSyncToast('上传失败：' + (e.message || '请检查网络'), 'error'); } finally { isSyncing = false; }
});

$('btnSyncDownload').addEventListener('click', () => { userDropdown.classList.remove('open'); if (!confirm('从云端下载将覆盖本地数据，确定继续？')) return; downloadFromCloud(); });

$('btnChangePassword').addEventListener('click', () => { userDropdown.classList.remove('open'); if (!currentUser) return; authUsername.value = window.CST.getDisplayName(currentUser); authUsername.disabled = true; openAuthModal('changePassword'); });

authUsername.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); authPassword.focus(); } });
authPassword.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); authMode === 'register' ? authConfirmPassword.focus() : authSubmitBtn.click(); } });
...
async function initAuth() {
    try { if (typeof window.CST === 'undefined') { setTimeout(initAuth, 500); return; } const user = await window.CST.getCurrentUser(); if (user) { onLoginSuccess(user, { download: true }); } } catch (e) { console.log('🔑 未登录（离线或首次使用）'); }
    try { window.CST.onAuthStateChange((event, session) => { if (event === 'SIGNED_OUT') { isLoggedIn = false; currentUser = null; lastSyncTime = null; authBtn.classList.remove('logged-in'); authBtn.title = '登录'; userDropdownSync.textContent = '○ 未同步'; userDropdownSync.className = 'user-dropdown-sync'; } else if (event === 'SIGNED_IN' && !isLoggedIn) { if (session?.user) onLoginSuccess(session.user); } }); } catch (e) {}
}
